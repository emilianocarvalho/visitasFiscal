CREATE SCHEMA IF NOT EXISTS `visitas` DEFAULT CHARACTER SET utf8 ;
USE `visitas` ;
...
CREATE TABLE IF NOT EXISTS `visitas`.`cidade` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_cidade_estabelecimento`
    FOREIGN KEY (`id_estabelecimento`)
    REFERENCES `visitas`.`estabelecimento` (`id`));
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_unicode_ci;

CREATE INDEX `fk_cidade_estabelecimento` ON `visitas`.`cidade` (`id_estabelecimento` ASC);
...
CREATE TABLE IF NOT EXISTS `visitas`.`auto` (
  ...
  `obs` VARCHAR(100) CHARACTER SET 'utf8' NULL DEFAULT NULL,
   ...
   CONSTRAINT `fk_id_estabelecimento`
    FOREIGN KEY (`id_estabelecimento`)
    REFERENCES `visitas`.`estabelecimento` (`id`)
    CONSTRAINT `fk_id_fiscal`
    FOREIGN KEY (`id_fiscal`)
    REFERENCES `visitas`.`fiscal` (`id`));
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COLLATE = utf8_unicode_ci;

CREATE INDEX `fk_cidade_estabelecimento` ON `visitas`.`estabelecimento` (`id_estabelecimento` ASC);
...
CREATE TABLE IF NOT EXISTS `visitas`.`estabelecimento` (
  ...
  `cnpj` INT(11) NULL DEFAULT NULL,
   PRIMARY KEY (`id`),
  CONSTRAINT `fk_estabelecimento_fiscal`
    FOREIGN KEY (`id_fiscal`)
    REFERENCES `visitas`.`fiscal` (`id`));
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
